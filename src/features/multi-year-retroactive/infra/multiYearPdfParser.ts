...
function extractMonthlyRecords(
  ...
  pagesItems.forEach(pageItems => {
    ...
    rowMap.forEach((rowItems) => {
      ...
      if (/IPARV|IPASGO|INSS|IRRF|CONSIGNADO|EMPR[EÉ]STIMO|DESCONTO|PENS[AÃ]O\s*ALIMENT|SINDICATO|MENSALIDADE|UNIMED|PLANO\s*DE\s*SA[UÚ]DE|VALE\s*TRANSPORTE|ADIANTAMENTO/i.test(rowText)) return;
